int printTriangle(int size) {
  ...
  ///for each number i that you count
    ...
    for (int j=0; j<=i;j++){  //for each number j that you count
...
int main(void) {
  ...
  numStars = printTriangle(7);//then call printTriangle, passing in 7, and assign the result to
  //numStars

  //finally, print "That triangle had %d total stars\n", such that
  printf("That triangle had %d total stars\n", numStars);//the %d prints the value of numStars
